...
SQLAdvisor는 Oracle AWR 리포트를 업로드하고, 텍스트 추출 및 구조화, SQL/Wait Event 분석, RAG 기반 Advisor Chat, Direct DB/직접 입력 기반 SQL 튜닝을 제공하는 웹 애플리케이션입니다.

| 영역 | 구성 | 주요 역할 |
| --- | --- | --- |
| Frontend | Vue 3 + Vite + TypeScript | 사용자 화면, 리포트 업로드/조회, 대시보드, Advisor Chat, SQL 튜닝, AI 설정 |
| Backend API | Java 17 + Spring Boot | REST API, AWR 파싱/분석, RAG 검색, SQL 튜닝, Target DB 연결, AI provider 연동 |
...
| REST API | Spring Web |
| Authentication | Spring Security, Google API Client, internal AD account identifier flow |
...
주요 API는 `/api` prefix를 사용하며, AWR 업로드, 리포트 조회, 분석, Chat, SQL 튜닝, Target DB 연결, AI 설정 API를 제공합니다.
...
| Structured Storage | `awr_report`, `awr_section`, `awr_sql_metric`, `awr_wait_event`, `target_db_connection`, `sql_tuning_result`, `sql_tuning_question` |
...
| Chat/LLM | OpenAI, Gemini, internal OpenAI-compatible endpoint, Ollama |
| Embedding | OpenAI, Gemini, internal OpenAI-compatible endpoint, Ollama |
| 설정 항목 존재 | Anthropic, xAI, Cohere rerank |

현재 실제 호출 구현은 OpenAI, Gemini, 내부 OpenAI-compatible endpoint, Ollama 중심이며, 외부 LLM이 비활성화되거나 호출에 실패하면 로컬 Advisor 결과로 동작합니다.
...
dev 모드는 Web, API, PostgreSQL, Redis 포트를 로컬에 노출하고, prod 모드는 Nginx 80 포트와 PostgreSQL 5432 포트를 외부에 노출하는 구성입니다.
