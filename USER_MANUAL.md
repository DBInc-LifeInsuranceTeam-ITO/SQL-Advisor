...
| 외부 LLM | 설정 상태 확인 | 현재 답변은 `rule-based-local-advisor` 기반입니다. |
| 데이터 영속성 | 제한적 | 원본 파일은 저장되지만, 리포트 목록과 분석 결과는 API 서버 재시작 시 초기화될 수 있습니다. |
...
A. 현재 리포트 목록과 분석 결과는 API 서버 메모리에 보관됩니다. API 서버가 재시작되면 목록이 초기화될 수 있습니다. 파일 자체는 저장소에 남아 있을 수 있지만, 다시 분석하려면 재업로드가 필요할 수 있습니다.
...
A. 필수는 아닙니다. 기본 로컬 규칙 기반 Advisor는 외부 API key 없이 동작합니다. AI 설정 화면의 provider 정보는 현재 구성 상태를 확인하기 위한 값입니다.
...
- 서버 재시작 후 데이터 초기화: 현재 메모리 저장 방식의 제약일 수 있으므로 재업로드 필요
