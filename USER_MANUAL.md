...
- Direct DB 또는 직접 입력 SQL 기반 SQL 튜닝 결과와 후속 질문 기록
...
| Direct DB SQL 튜닝 | 지원 | 저장된 Oracle 연결에서 Top SQL 후보를 불러오고 실행계획, 기존 인덱스, bind 근거를 수집합니다. |
| 직접 입력 SQL 튜닝 | 지원 | SQL text, 실행계획, DDL, 기존 인덱스, bind 정보를 직접 입력해 튜닝 결과를 생성합니다. |
...
SQL 튜닝 화면은 Direct DB에서 SQL_ID 근거를 수집하거나, SQL text와 실행계획을 직접 입력해 튜닝 결과를 생성하는 화면입니다.

#### Direct DB 사용 방법

1. `Direct DB` 모드를 선택합니다.
2. 기존 연결을 선택하거나 새 연결 정보를 입력합니다.
3. 새 연결은 `Test`로 접속 가능 여부와 조회 권한을 확인한 뒤 저장합니다.
4. `Load SQL`로 Top SQL 후보를 불러옵니다.
5. 필요하면 기준 소스, 정렬 기준, 시간 범위, schema/module/program 필터를 조정합니다.
6. SQL_ID를 선택하면 SQL text, 실행계획, 기존 인덱스, bind sample 근거가 채워집니다.
7. `Tune Direct DB`를 실행하고 요약, 증상, 인덱스 후보, rewrite, 검증 SQL을 확인합니다.

Direct DB의 Top SQL 목록은 전체 SQL 목록이 아니라, 설정한 기준과 조회 가능한 Oracle view 범위에서 필터링된 튜닝 후보입니다. 이미 튜닝한 SQL_ID는 `Hide tuned SQL_ID` 토글로 숨길 수 있습니다.

#### 직접 입력 사용 방법

1. `Manual SQL` 모드를 선택합니다.
2. SQL text를 입력합니다.
3. 가능하면 실행계획, schema DDL, 기존 인덱스, bind sample을 함께 입력합니다.
4. `Tune SQL`을 실행하고 결과의 권장 조치와 검증 SQL을 확인합니다.

#### 후속 질문

튜닝 결과가 생성되면 화면 하단에서 해당 결과를 기준으로 후속 질문을 남길 수 있습니다. 질문과 답변은 선택한 튜닝 결과의 히스토리로 저장됩니다.
...
AI 설정 화면에서는 LLM provider, embedding provider, 모델, API key 설정 상태를 확인합니다.

#### 확인할 수 있는 정보

- 현재 Chat/LLM provider와 embedding provider
- provider별 API key 설정 여부와 key 출처
- provider별 chat model, embedding model, base URL
- 누락된 key 또는 설정 경고

#### 사용 방법

1. Chat에 사용할 provider와 embedding에 사용할 provider를 선택합니다.
2. 환경 변수 값을 그대로 사용할지, 화면에서 직접 입력할지 선택합니다.
3. 필요한 모델, API key, base URL을 입력합니다.
4. `저장`을 클릭합니다. 운영 설정에 따라 저장은 관리자 권한이 필요할 수 있습니다.

외부 또는 내부 LLM이 비활성화되거나 호출에 실패하면 기본 로컬 규칙 기반 Advisor 결과로 동작합니다.
...
### Direct DB에서 SQL_ID 튜닝하기

1. `/sql-tuning`에서 Direct DB 연결을 선택합니다.
2. `Load SQL`로 후보 SQL_ID를 불러옵니다.
3. SQL_ID를 클릭해 실행계획과 주변 근거를 수집합니다.
4. `Tune Direct DB`를 실행합니다.
5. 인덱스 후보가 제안되면 DDL을 바로 적용하지 말고 테스트 환경에서 검증 SQL과 실행계획 변화를 먼저 확인합니다.
...
### Q. Direct DB에서 `Load SQL`을 눌렀는데 후보가 비어 있습니다.

A. 선택한 source, 시간 범위, schema/module/program 필터에서 조회되는 후보가 없거나, 연결 계정이 필요한 Oracle 성능 view를 읽지 못하는 경우일 수 있습니다. 먼저 필터를 넓히고 저장된 연결의 권한과 테스트 결과를 확인하세요.
...
이 매뉴얼은 현재 SQLAdvisor 웹 애플리케이션의 AWR 업로드, 분석, Advisor Chat, SQL 튜닝 흐름을 기준으로 작성되었습니다.
